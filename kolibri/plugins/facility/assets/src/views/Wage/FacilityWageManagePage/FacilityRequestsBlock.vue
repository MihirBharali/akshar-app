...
<script>
  ...
  import FacilityWageFormModal from './FacilityWageFormModal.vue';

  export default {
    name: 'FacilityRequestsBlock',
    components: { PaginatedListContainer, CoreTable, FacilityWageFormModal },
    ...
    methods: {
      createWageRequest({ name, id, amount, requestType, reason, description }) {
        this.$store
          .dispatch('wage/createWageRequestForLearner', {
            amount: amount,
          ...
          .then(response => {
            ...
            this.$store.commit('wage/UPDATE_FACILITY_WAGE_REQUEST_LIST', response);
